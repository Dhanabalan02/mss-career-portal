...
def update_job_post(
    ...
    if pre_screening_questions is not None:
        db.query(JobPreScreeningQuestion).filter(JobPreScreeningQuestion.job_id == job_id).delete()
...
def clone_job_post(db: Session, job_id: int, admin_id: int, publish: bool = False) -> JobPost:
    ...
    original_questions = db.query(JobPreScreeningQuestion).filter(JobPreScreeningQuestion.job_id == job_id).all()
...
def get_job_prescreening_questions(db: Session, job_id: int) -> List[JobPreScreeningQuestion]:
    """Retrieves all pre-screening questions for a specific job post."""
    return db.query(JobPreScreeningQuestion).filter(JobPreScreeningQuestion.job_id == job_id).all()
