...
def get_hr_reports(
    ...
    open_jobs = jobs_q.filter(JobPost.job_status == "open").count()
    closed_jobs = jobs_q.filter(JobPost.job_status != "open").count()
    ...
    school_comparison = {row[0]: row[1] for row in school_hires if row[0]}
    if not school_comparison:
        school_comparison = {"ABC Public School": 0, "XYZ International": 0}

    # Vacancy Gap Analysis
    dept_vacancies_q = (
        db.query(JobPost.department, func.sum(JobPost.vacancy_count))
        .filter(JobPost.job_status == "open")
    )
    if not is_hr:
    ...
    vacancy_gap = {row[0]: row[1] or 0 for row in dept_vacancies if row[0]}
    if not vacancy_gap:
        vacancy_gap = {"Teaching Staff": 0, "Administration": 0, "Arts & Music": 0}
