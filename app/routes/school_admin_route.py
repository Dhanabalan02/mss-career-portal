...
def issue_offer_route(
    ...
    if not payload.is_draft:
        try:
            ...
            app_record = db.query(JobApplicant).filter(JobApplicant.job_applicant_id == applicant_id).first()
            if app_record:
                job = db.query(JobPost).filter(JobPost.job_id == app_record.job_id).first()
                candidate = db.query(Users).filter(Users.user_id == app_record.user_id).first()
                ...
                if job and candidate:
                    ...
                    try:
                        offer_logger.info(f"Initiating WhatsApp OfferService for candidate_id={candidate.user_id}")
                        from app.services.offer_service import OfferService
                        phone = candidate.mobile
                        
                        candidate_name = getattr(candidate, "name", getattr(candidate, "first_name", "Candidate"))
                        
                        offer_logger.info(f"Candidate phone resolved to: {phone}")
                        if phone:
                            doc_url = app_record.offer_letter_doc_path
                            if not doc_url:
                                offer_logger.warning(f"No offer document path for candidate {candidate.user_id}, skipping WhatsApp offer.")
                            else:
                                doc_url = doc_url.replace("\\", "/")
                                if not doc_url.startswith(("http://", "https://")):
                                    base_url = "https://stagecareer.themadrassevasadan.org"
                                    doc_url = base_url + (doc_url if doc_url.startswith("/") else f"/{doc_url}")
                                ...
                                offer_logger.info(f"Offer document URL: {doc_url}, Job Title: {job.job_title}")
                                ...
                                offer_logger.info(f"OfferService response for candidate_id={candidate.user_id}: {response}")
                                if response.get("success"):
                                    offer_logger.info("OfferService().issue_offer executed successfully")
                                else:
                                    offer_logger.error(f"OfferService failed for candidate_id={candidate.user_id}. Details: {response}")
                        else:
                            offer_logger.warning(f"Candidate {candidate.user_id} has no phone number, WhatsApp offer skipped.")
                    except Exception as e:
                        offer_logger.error(f"Error calling WhatsApp service for candidate_id={candidate.user_id}: {e}", exc_info=True)
        except Exception as e:
            logger.error(f"Error creating notification for issued offer: {e}", exc_info=True)
...
def update_offer_status_via_webhook(
    ...
    # Clean phone formatting matching your normalization logic
    phone_clean = payload.phone.replace("+91", "").strip() 
    
    candidate = db.query(Users).filter(
        (Users.phone.contains(phone_clean)) | (Users.mobile.contains(phone_clean))
    ).first()
    
    if not candidate:
        raise HTTPException(status_code=44, detail="Candidate not found by phone reference")
        
    app = db.query(JobApplicant).filter(
        JobApplicant.user_id == candidate.user_id
    ).order_by(JobApplicant.job_applicant_id.desc()).first() # Gets latest open application
    
    if not app:
        raise HTTPException(status_code=44, detail="No active application found for candidate")
