...
def _get_admin_id_from_token(authorization: Optional[str] = Header(default=None)) -> int:
...
class JobPostCreateRequest(BaseModel):
    job_posted_by: Optional[int] = None  # Ignored — admin_id is derived from the JWT token
...
def _serialize_job_post(job_post: JobPost, db: Optional[Session] = None) -> dict:
    is_clone = False
    if db:
        is_clone = db.query(JobPost).filter(
    ...
    return {
        ...
        ] if job_post.job_pre_screening_questions else [],
    }
...
def get_public_job_post_by_id_route(
    job_id: int,
    db: Session = Depends(get_db)
):
...
def get_applicants_route(
    ...
    is_hr = admin is not None and admin.user_roles.role_name in {"hr_head", "hr_team", "hr_admin"}
    ...
    colors = ["#378ADD", "#534AB7", "#1D9E75", "#EF9F27", "#D85A30", "#0891b2", "#993C1D", "#0F6E56"]
    ...
    for idx, app in enumerate(all_applicants):
        ...
        if search:
            ...
            if search_lower not in name.lower() and search_lower not in job_title.lower():
                continue
        if stage and stage != stage_val:
            continue
        if exp:
            if exp == "0-3 yrs" and not (0 <= exp_years <= 3):
                continue
            elif exp == "3-6 yrs" and not (3 < exp_years <= 6):
                continue
            elif exp == "6-10 yrs" and not (6 < exp_years <= 10):
                continue
            elif exp == "10+ yrs" and not (exp_years > 10):
                continue
...
def get_applicant_detail_route(
    ...
    # Serialize education
    edu_records = db.query(CandidateEducationDetail).filter(CandidateEducationDetail.user_id == user.user_id).all()
    education_list = []
    ...
    screening_answers = db.query(CandidateScreeningAnswer).filter(
...
def update_applicant_status_route(
    ...
    if status_lower == "selected":
        ...
        raise HTTPException(status_code=400, detail=f"Invalid status value: {form_data.status}")
...
def get_job_post_by_id_route(
    ...
    applications_count = db.query(JobApplicant).filter(JobApplicant.job_id == job_id).count()
...
def create_job_post_route(
    ...
    job_post = create_job_post(
        ...
        pre_screening_questions=(
            ...
            if form_data.pre_screening_questions else None
        ),
    )
...
def update_job_post_route(
    job_id: int,
    form_data: JobPostUpdateRequest,
    db: Session = Depends(get_db)
):
    ...
    job_post = update_job_post(
        ...
        pre_screening_questions=(
            [question.model_dump() for question in form_data.pre_screening_questions]
            if form_data.pre_screening_questions is not None else None
        ),
    )
    return _serialize_job_post(job_post, db=db)
...
def clone_job_post_route(
    ...
    cloned_job = clone_job_post(db=db, job_id=job_id, admin_id=admin_id, publish=publish)
    return _serialize_job_post(cloned_job, db=db)


@router.patch("/{job_id}/draft")
def draft_job_post_route(
    job_id: int,
    db: Session = Depends(get_db)
):
...
def get_job_prescreening_questions_route(
    job_id: int,
    db: Session = Depends(get_db)
):
...
def get_dashboard_stats(
    ...
    onboarded_candidates = base_q.filter(JobApplicant.issue_appointment_order == 1).count()
    ...
    if not is_hr:
        interviewed_subq_q = interviewed_subq_q.filter(JobPost.job_posted_by == admin_id)
    ...
    return {
        ...
        "funnel": [
            ...
            {"stage": "Offer Accepted", "count": offer_accepted_count, "color": "#1D9E75"},
...
def get_dashboard_recent_applicants(
    ...
    rows = (
        query.order_by(JobApplicant.job_applicant_id.desc())
        .limit(10)
        .all()
    )
    ...
    for idx, (app, user, job_post) in enumerate(rows):
        ...
        if app.sync_masset == 1:
            ...
            interview_status = "Offer Accepted"
        elif app.offer_acceptance_status in (OfferAcceptanceStatus.EXPIRED, OfferAcceptanceStatus.REJECTED):
            stage = "Offer"
            interview_status = "Offer " + (app.offer_acceptance_status.value.capitalize())
